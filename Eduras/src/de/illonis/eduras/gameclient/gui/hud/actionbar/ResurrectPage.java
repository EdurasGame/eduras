...
public class ResurrectPage extends ActionBarPage {
	...
	public void onPlayerJoined(int ownerId) {
		...
		try {
			PlayerMainFigure joinedPlayer = infoPro.getPlayerByOwnerId(ownerId);
			PlayerMainFigure player = infoPro.getPlayer();
			if (infoPro.getGameMode().getRelation(joinedPlayer, player) == Relation.ALLIED) {
